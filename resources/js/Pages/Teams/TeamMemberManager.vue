  ...
  <div>
    <div v-if="userPermissions.canAddTeamMembers">
      ...
      <jet-form-section @submitted="addTeamMember">
        ...
        <template #form>
          ...
          <!-- Member Email -->
          <div class="col-span-6 sm:col-span-4 relative label-floating">
            <jet-input id="name" type="text" class="block w-full" v-model="addTeamMemberForm.email" />
            <jet-label for="email" value="Email" />
            <jet-input-error :message="addTeamMemberForm.error('email')" class="mt-1" />
          </div>
        ...
        <template #actions>
          ...
          <jet-button :class="{ 'opacity-50 hover:opacity-50': addTeamMemberForm.processing }" :disabled="addTeamMemberForm.processing">
            Add
          </jet-button>
        </template>
      </jet-form-section>
    ...
    <jet-dialog-modal :show="currentlyManagingRole" @close="currentlyManagingRole = false">
      ...
      <template #footer>
        ...
        <jet-button class="ml-2" @click.native="updateRole" :class="{ 'opacity-50 hover:opacity-50': updateRoleForm.processing }" :disabled="updateRoleForm.processing">
          Save
        </jet-button>
      </template>
    </jet-dialog-modal>
    ...
    <jet-confirmation-modal :show="confirmingLeavingTeam" @close="confirmingLeavingTeam = false">
      ...
      <template #footer>
        ...
        <jet-danger-button class="ml-2" @click.native="leaveTeam" :class="{ 'opacity-50 hover:opacity-50': leaveTeamForm.processing }" :disabled="leaveTeamForm.processing">
          Leave
        </jet-danger-button>
      </template>
    ...
    <jet-confirmation-modal :show="teamMemberBeingRemoved" @close="teamMemberBeingRemoved = null">
      ...
      <template #footer>
        ...
        <jet-danger-button class="ml-2" @click.native="removeTeamMember" :class="{ 'opacity-50 hover:opacity-50': removeTeamMemberForm.processing }" :disabled="removeTeamMemberForm.processing">
          Remove
        </jet-danger-button>
      </template>
    </jet-confirmation-modal>
